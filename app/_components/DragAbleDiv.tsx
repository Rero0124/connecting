...
export default function DragAbleDiv ({ children, option, classname = '', style, onDragging, onDragEnd }: {
  ...
  const moveX = option.direction.indexOf('left') > -1 || option.direction.indexOf('start') > -1 || option.direction === 'left' || option.direction === 'start' || option.direction.indexOf('right') > -1 || option.direction.indexOf('end') > -1 || option.direction === 'right' || option.direction === 'end';
  const moveY = option.direction.indexOf('top') > -1 || option.direction === 'top' || option.direction.indexOf('bottom') > -1 || option.direction === 'bottom';
  
  if(!(moveX || moveY)) new Error('invaild dragAbleDiv param - option > direction');
  ...
  function mouseMoveHandler(e: MouseEvent) {
    if(ref.current) {
      ...
      if(moveY) {
        const nPosY = minHeight > dy ? minHeight : maxHeight < dy ? maxHeight : dy;
  ...
  return (
    <div ref={ref} className={classname} style={{ ...basicStyle, ...style }} onMouseDown={setPreventDefault} onDrag={setPreventDefault}>
      <div className="flex flex-row w-full h-full">
        <div ref={childRef} className="grow">
          {children}
        </div>
        <div className={'border-r-[1px]' + (option.hoverColor && ` hover:bg-${option.hoverColor} hover:border-${option.hoverColor}`)} style={{ width: hoverSize, cursor: 'w-resize' }}>
