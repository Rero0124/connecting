...
export default function FriendList() {
    ...
    useEffect(() => {
        if (
            !(
                ...
                selectedFriendSubMenu === "favorite"
            )
        ) {
    ...
    function Menu({children, name, classname = ""}: {children: React.ReactNode; name: string; classname?: string}) {
        ...
        return (
            <div
                className={`hover:bg-background-light w-22 h-9 p-2 ml-2 my-2 rounded text-center leading-6 ${classname}`}
                onClick={onClick}
            >
                {children}
            </div>
    ...
    return (
        <div className="flex flex-col">
            <div className="flex flex-row h-13 border-b-[1px]">
                <Menu
                    name="all"
                    classname={selectedFriendSubMenu === "all" ? "bg-background-light" : "hover:bg-background-light"}
                >
                    모두
                </Menu>
                <Menu
                    name="online"
                    classname={selectedFriendSubMenu === "online" ? "bg-background-light" : "hover:bg-background-light"}
                >
                    온라인
                </Menu>
                <Menu
                    name="favorite"
                    classname={
                        selectedFriendSubMenu === "favorite" ? "bg-background-light" : "hover:bg-background-light"
                    }
                >
                    친한친구
                </Menu>
