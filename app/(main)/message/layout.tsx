...
export default function Layout({
  ...
  return (
    <>
      <DragAbleDiv classname="bg-background border-r-[1px]" option={dragAbleDivOption} onDragging={onDragging} onDragEnd={onDragEnd}>
        <div ref={navRef} className="bg-background flex flex-col h-full pl-2.5 pr-1 py-2" style={{ width: navSize }}>
          <Menu name="allow" classname={selectedMessageMenu === 'allow' ? 'bg-background-light' : 'hover:bg-background-light'}>
            메세지 요청 & 스팸
          </Menu>
          <hr className="" />
          ...
          <Menu name="send" classname="flex h-auto">
