...
export default function ChangeProfileModal({open, profiles, onClose, onSelect, onLoginAnotherAccount}: Props) {
    if (!open) return null;

    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
            <div className="relative bg-background p-8 rounded-lg w-[500px] border border-gray-700">
                <button onClick={onClose} className="absolute top-3 right-3 text-gray-300 hover:text-white text-xl">
                    ×
                </button>
                ...
                <div className="flex flex-col gap-3">
                    {profiles && profiles.length > 0 ? (
                        ...
                        <p className="text-gray-400 text-center">사용 가능한 프로필이 없습니다.</p>
                    )}
                </div>
