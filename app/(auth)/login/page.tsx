...
export default function Login() {
  ...
  const [selectProfileModelOpen, setSelectProfileModelOpen] = useState<boolean>(false);
  ...
  return (
    <>
      <form ref={formRef} action={action} className="relative flex flex-col p-12 border-[1px] rounded-lg">
        <input ref={profileIdRef} name="profileId"/>
        {
          selectProfileModelOpen && <div className="absolute top-0 left-0 w-full h-full"></div>
        }
        ...
        <div className="flex flex-row justify-between min-w-80 h-12 px-3 py-1 leading-10">
          ...
          <input id="email" className="p-2 border-[1px] rounded-lg" name="email" placeholder="Email" defaultValue={ state?.data?.email } />
        </div>
        <div className="flex flex-row justify-between min-w-80 h-12 px-3 py-1 leading-10">
          <label htmlFor="password">Password</label>
          <input id="password" className="p-2 border-[1px] rounded-lg" name="password" type="password" placeholder="Password" defaultValue={ state?.data?.password } />
        </div>
        {state?.errors?.email && <p>{state.errors.email}</p>}
        {state?.message && <p>{state.message}</p>}
        <div className="flex flex-row justify-between px-3 pt-8">
          <Link className="cursor-pointer" href="/join">Join</Link>
          <button className="cursor-pointer" disabled={pending} type="submit">Login</button>
        </div>
      </form>
      { 
        selectProfileModelOpen && <>
          <div className="absolute flex flex-col justify-center items-center p-12 border-[1px] rounded-lg bg-background">
            <div className="absolute top-0 right-0 p-3 cursor-pointer" onClick={cancelLogin}>X</div>
            <div className="h-14 mb-4 text-4xl text-center">
              <p>Select Profile</p>
            </div>
            <div className="flex flex-col min-w-60">
              {
                state && state.profiles ? state.profiles.map((profile) => {
                  return (
