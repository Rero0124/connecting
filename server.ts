...
app.prepare().then(() => {
    ...
    const findSocketIdByUserId = (userId: number | number[]) =>
        socketArr.find((socket) =>
            userId instanceof Array ? userId.indexOf(socket.userId) > -1 : socket.userId === userId
        );
    const socktArrDeleteByUserId = (userId: number | number[]) => {
        socketArr.splice(
            socketArr.findIndex((socket) => socket.userId === userId),
            1
        );
    };
    const socktArrDeleteBySessionId = (socketId: string | string[]) => {
        socketArr.splice(
            socketArr.findIndex((socket) => socket.id === socketId),
            1
        );
    };
