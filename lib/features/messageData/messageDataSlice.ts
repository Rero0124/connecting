...
export const getAllowedMessages = (state: RootState, messageId?: string) => {
	if (messageId) {
		return state.messageData.allowedMessages.find(
			(allowedMessage) => allowedMessage.id === messageId
		)
	} else {
...
export const getNotAllowedMessages = (state: RootState, messageId?: string) => {
	if (messageId) {
		return state.messageData.notAllowedMessages.find(
...
export const getMessageChat = (state: RootState, messageId: string) => {
	return state.messageData.messageChats.find(
		(messageChat) => messageChat.messageId === messageId
	)
}
